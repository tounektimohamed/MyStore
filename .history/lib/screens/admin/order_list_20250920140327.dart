...
Future<void> _exportCSV() async {
  ...
  try {
    ...
    // En-têtes avec BOM UTF-8 pour une meilleure compatibilité
    List<List<dynamic>> csvData = [
      [utf8.decode([0xEF, 0xBB, 0xBF]), ''], // BOM UTF-8
      ['ID', 'Client', 'Email', 'Produit', 'Quantité', 'Prix', 'Statut', 'Date']
    ];

    // Récupération de tous les produits en une seule fois pour optimiser
    ...
    for (var order in filteredOrders) {
      ...
      final productName = product?.name ?? 'Inconnu';
      final price = (product?.price ?? 0) * order.quantity;
      
      csvData.add([
        order.id,
        _escapeCsvField(order.customerName),
        _escapeCsvField(order.customerEmail),
        _escapeCsvField(productName),
        order.quantity,
        price.toStringAsFixed(2).replaceAll('.', ','), // Format européen
        order.status,
    ...
    final csv = const ListToCsvConverter(
      fieldDelimiter: ';', // Séparateur point-virgule (standard européen)
      textEndDelimiter: '"', // Guillemets pour les champs texte
    ).convert(csvData);

    // Export selon la plateforme
    if (kIsWeb) {
      // Web : export avec BOM UTF-8
      ...
      // Optionnel : ouvrir le fichier (Android/iOS)
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(
              label: 'Ouvrir',
              onPressed: () {
                // Vous pourriez utiliser un package comme 'open_file' ici
                // OpenFile.open(path);
              },
            ),
