...
class _AdminOrderListState extends State<AdminOrderList> {
    ...
    return StreamBuilder<List<Order>>(
      ...
      builder: (context, snapshot) {
        ...
        final pendingOrders = orders
            .where((o) => o.status.toLowerCase() == 'en attente')
            .length;
  ...
  List<Order> _filterAndSortOrders(List<Order> orders) {
    ...
    var filteredOrders = orders.where((order) {
      ...
      if (_selectedFilter == 'Toutes') {
        ...
        matchesFilter = order.status.toLowerCase() == 'confirmed' || 
                        order.status.toLowerCase() == 'confirmée';
      } else {
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      // Note: FileSaver.instance.saveFile n'est plus disponible dans les versions récentes
      // Utilisez plutôt la méthode suivante pour partager le fichier
      // await Share.shareFiles([path], text: 'Export des commandes');
