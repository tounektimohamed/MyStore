...
class _AdminOrderListState extends State<AdminOrderList> {
    ...
    return orders.where((order) {
      ...
      final matchesSearch =
          ...
          order.customerPhone.toLowerCase().contains(
            _searchQuery,
          ) || // Changé: téléphone au lieu d'email
          order.id.toLowerCase().contains(_searchQuery);

      // Filtre par statut
      final matchesStatus =
          _selectedStatus == 'Tous' || order.status == _selectedStatus;
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              title: Text(
                'Du: ${_startDate != null ? DateFormat('dd/MM/yyyy').format(_startDate!) : 'Sélectionner'}',
              ),
              onTap: () async {
            ...
            ListTile(
              leading: Icon(Icons.calendar_today),
              title: Text(
                'Au: ${_endDate != null ? DateFormat('dd/MM/yyyy').format(_endDate!) : 'Sélectionner'}',
              ),
              onTap: () async {
                final picked = await showDatePicker(
                  context: context,
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      // Nom du fichier
      final fileName =
          'commandes_${DateFormat('yyyy-MM-dd_HHmmss').format(DateTime.now())}.csv';
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(
              Icons.location_on,
              'Adresse (Rue): ${order.customerAddress}',
            ), // Modifié
            _buildInfoRow(
              Icons.flag,
