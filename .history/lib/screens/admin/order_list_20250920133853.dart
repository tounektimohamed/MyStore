    ...
    return StreamBuilder<List<Order>>(
      ...
      builder: (context, snapshot) {
        ...
        final confirmedOrders = orders
            .where((o) => o.status.toLowerCase() == 'confirmed' || o.status.toLowerCase() == 'confirmée')
            .length;
        final pendingOrders = orders.where((o) => o.status.toLowerCase() == 'en attente').length;
  ...
  List<Order> _filterAndSortOrders(List<Order> orders) {
    var filteredOrders = orders.where((order) {
      ...
      if (_selectedFilter == 'Toutes') {
        matchesFilter = true;
      } else if (_selectedFilter == 'Confirmées') {
        matchesFilter = order.status.toLowerCase() == 'confirmed' || order.status.toLowerCase() == 'confirmée';
      } else {
        matchesFilter = order.status.toLowerCase() == _selectedFilter.toLowerCase();
      }

      final orderDate = order.orderDate;
      final matchesDate =
          (_startDate == null || orderDate.isAfter(_startDate!)) &&
        ...
        content: Column(
          ...
          children: [
            ListTile(
              ...
              title: Text(
                'Du ${_startDate != null ? DateFormat('dd/MM/yyyy').format(_startDate!) : '...'}',
              ),
              onTap: () async {
            ...
            ListTile(
              leading: Icon(Icons.calendar_today, color: Colors.blue[700]),
              title: Text(
                'Au ${_endDate != null ? DateFormat('dd/MM/yyyy').format(_endDate!) : '...'}',
              ),
              onTap: () async {
                final picked = await showDatePicker(
                  context: context,
            ...
            children: [
              Row(
                ...
                children: [
                  Flexible(
                    child: Text(
                      'Commande #${order.id.substring(0, order.id.length > 8 ? 8 : order.id.length).toUpperCase()}',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        fontSize: 16,
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  _buildInfoRow(Icons.calendar_today, DateFormat('dd/MM/yyyy').format(order.orderDate)),
                  _buildInfoRow(Icons.access_time, DateFormat('HH:mm').format(order.orderDate)),
                ],
              ),
            ],
          ),
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Fichier CSV exporté avec succès: $path'),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 16),
          Text('Aucune commande trouvée', style: TextStyle(fontSize: 18, color: Colors.grey[600])),
        ],
      ),
    );
  }
