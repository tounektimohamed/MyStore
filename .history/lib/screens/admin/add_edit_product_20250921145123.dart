...
class _AdminOrderListState extends State<AdminOrderList> {
  ...
  Future<void> _exportCSV() async {
    ...
    try {
      ...
      List<List<dynamic>> csvData = [
        ['ID_Commande', 'Nom_Client', 'Telephone', 'Produit', 'Quantite', 'Prix_Total', 'Statut', 'Date_Commande', 'Adresse_Rue', 'Region', 'CodePostal']
      ];
      ...
      for (var order in filteredOrders) {
        ...
        csvData.add([
          _escapeForCSV(order.id),
          _escapeForCSV(order.customerName),
          _escapeForCSV(order.customerPhone),
          _escapeForCSV(productName),
          order.quantity.toString(),
          totalPrice.toStringAsFixed(2).replaceAll('.', ','),
          _escapeForCSV(order.status),
          _escapeForCSV(dateFormat.format(order.orderDate)),
          _escapeForCSV(order.customerAddress),
          _escapeForCSV(order.customerRegion),
          _escapeForCSV(order.customerCodePostal),
        ]);
      }
  ...
  String _escapeForCSV(String value) {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow(Icons.phone, 'Téléphone: ${order.customerPhone}'),
            _buildInfoRow(Icons.location_on, 'Adresse: ${order.customerAddress}'),
            _buildInfoRow(Icons.flag, 'Région: ${order.customerRegion}'),
            _buildInfoRow(Icons.flag, 'Code Postal: ${order.customerCodePostal}'),
